...
class SslCommerzNotification extends AbstractSslCommerz
{
    ...
    {
        $this->config = Config::get('paymentmethods.sslcommerz');

        $this->setStoreId($this->config['store_id']);
        $this->setStorePassword($this->config['store_passwd']);
    }
...
//                    }
    ...
    public function makePayment(array $requestData, $type = 'checkout', $pattern = 'json')
    {
        ...
        $response = $this->callToApi($requestData, $header, $this->config['connect_from_localhost']);
